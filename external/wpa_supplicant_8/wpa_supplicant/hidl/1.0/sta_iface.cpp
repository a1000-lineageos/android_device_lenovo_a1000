...
StaIface::getMacAddressInternal()
{
	...
	if (ret < 0 || reply_str.empty() ||
		...
		// A1000/sprdwl приватной команды MACADDR не знает, и «MAC-адрес
		// Wi-Fi» в настройках оставался «Недоступно». Адрес интерфейса
		// wpa_supplicant всё равно держит в own_addr (читает при
		// инициализации), отдаём его.
		int i;
		for (i = 0; i < ETH_ALEN; i++) {
			if (wpa_s->own_addr[i] != 0)
				break;
		}
		if (i < ETH_ALEN) {
			std::array<uint8_t, 6> own_mac;
			os_memcpy(own_mac.data(), wpa_s->own_addr, ETH_ALEN);
			return {{SupplicantStatusCode::SUCCESS, ""}, own_mac};
		}
		return {{SupplicantStatusCode::FAILURE_UNKNOWN, ""}, {}};
	}
