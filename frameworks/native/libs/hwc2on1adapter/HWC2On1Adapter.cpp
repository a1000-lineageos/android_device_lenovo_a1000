...
bool HWC2On1Adapter::prepareAllDisplays() {
    ...
    // оставались одни обои. Один оверлей работает исправно, поэтому режем
    // только случай "их больше одного": возвращаем такие слои на GPU и просим
    // блоб подготовиться заново. HWC_SKIP_LAYER здесь обязателен — по
    // контракту HWC1 слой с этим флагом должен остаться HWC_FRAMEBUFFER,
    // иначе повторный prepare снова разложил бы всё по оверлеям.
    bool a1000NeedRepeat = false;
    for (auto& contents : mHwc1Contents) {
        if (!contents || contents->numHwLayers == 0) {
            continue;
        }
        // последний слой — HWC_FRAMEBUFFER_TARGET, он не в счёт
        size_t numLayers = contents->numHwLayers - 1;
        size_t overlays = 0;
        ...
        a1000NeedRepeat = true;
    }
    if (a1000NeedRepeat) {
        ATRACE_NAME("HWC1 prepare (A1000: повтор без оверлеев)");
        mHwc1Device->prepare(mHwc1Device, mHwc1Contents.size(),
                mHwc1Contents.data());
    }
