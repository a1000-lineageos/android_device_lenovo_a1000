        ...
        uint32_t* outStride, buffer_handle_t* outBufferHandles) const
{
    ...
    auto ret = mAllocator->allocate(descriptor, count,
            [&](const auto& tmpError, const auto& tmpStride,
                ...
                if (tmpError != Error::NONE) {
                    // A1000: шаг 2 — отказал САМ сервис аллокатора.
                    ALOGE("A1000: allocate: сервис аллокатора отказал, код %d",
                          static_cast<int>(tmpError));
                    return;
                }
                ...
                for (uint32_t i = 0; i < count; i++) {
                    error = mMapper.importBuffer(tmpBuffers[i],
                            &outBufferHandles[i]);
                    if (error != Error::NONE) {
                        // A1000: шаг 3 — сервис буфер отдал, но импорт его в
                        // ЭТОТ процесс не удался. Такого шага на Android 5 не
                        // существовало вовсе.
                        ALOGE("A1000: allocate: importBuffer #%u отказал, код %d",
    ...
    if (!ret.isOk()) {
        // A1000: sama HIDL-tranzakciya ne proshla. Naruzhu eto uhodit tem zhe
        // kodom NO_RESOURCES (kTransactionError), chto i otkaz gralloc, i
        // otlichit' ih po kodu nevozmozhno - poetomu razvodim yavno.
        ALOGE("A1000: allocate: tranzakciya k servisu ne proshla: %s",
