...
status_t HWComposer::prepare() {
    ...
    // A1000: не отдаём блобу больше одного оверлея.
    // hwcomposer.sc8830 при двух полноэкранных OSD-слоях и отсутствии видео
    // забирает в аппаратный путь оба, отдаёт их в GSP и рапортует
    // "GSP_Proccess success", но верхний слой с альфой в результат не попадает
    // — так пропадали экран блокировки и «недавние приложения»: на панели
    // оставались одни обои, хотя screencap показывал полный кадр.
    //
    // Помечаем слои ДО prepare, а не разбираем его результат: если звать
    // prepare второй раз, блоб оставляет плоскость настроенной под слой обоев
    // (RGBX_8888, pitch 960) и выводит по этим параметрам FB TARGET
    // (RGBA_8888, pitch 480) — на переходах проскакивает кадр с переставленными
    // красным и синим, сине-зелёные обои желтеют. Один проход этого не даёт.
    //
    // HWC_SKIP_LAYER обязателен: по контракту HWC1 слой с этим флагом остаётся
    // HWC_FRAMEBUFFER, то есть композируется GPU. Один оконный слой не трогаем
    // — аппаратный путь для него исправен и нужен, в частности, для видео.
    Vector< std::pair<size_t, size_t> > forcedToGles;
    for (size_t i = 0 ; i < mNumDisplays ; i++) {
        DisplayData& disp(mDisplayData[i]);
        if (disp.list == NULL) {
            continue;
        }
        // последний слой списка — HWC_FRAMEBUFFER_TARGET, он не оконный
        size_t windowLayers = disp.list->numHwLayers;
        if (disp.framebufferTarget != NULL && windowLayers > 0) {
            windowLayers--;
        }
        // Гибрид: один слой отдаём аппаратному пути (он с этим справляется и
        // экономит GPU), а два и более — композируем сами. Блоб при двух
        // полноэкранных OSD-слоях забирает в оверлей оба, отдаёт их в GSP,
        // рапортует "GSP_Proccess success", но верхний слой с альфой в
        // результат не попадает — так пропадали экран блокировки и «недавние».
        // Перестановка красного и синего в оверлее лечится отдельно, битом
        // rb_switch в DISPC (sprdfb_dispc.osd_rb_ov).
        if (windowLayers < 2) {
            continue;
        }
        for (size_t l = 0 ; l < windowLayers ; l++) {
            hwc_layer_1_t& layer = disp.list->hwLayers[l];
            if ((layer.flags & HWC_SKIP_LAYER) == 0) {
                layer.flags |= HWC_SKIP_LAYER;
                forcedToGles.add(std::make_pair(i, l));
            }
        }
    }
    ...
    // Снимаем ТОЛЬКО свои флаги: список слоёв переиспользуется между кадрами,
    // а решение должно приниматься заново на каждом кадре.
    for (size_t k = 0 ; k < forcedToGles.size() ; k++) {
        DisplayData& disp(mDisplayData[forcedToGles[k].first]);
        if (disp.list == NULL) {
            continue;
        }
        disp.list->hwLayers[forcedToGles[k].second].flags &= ~HWC_SKIP_LAYER;
    }
