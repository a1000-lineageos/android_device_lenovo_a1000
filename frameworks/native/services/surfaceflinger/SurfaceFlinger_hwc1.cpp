...
#include <dlfcn.h>
#include <errno.h>
/* A1000: framebuffer pan for the GLES-only path */
#include <fcntl.h>
#include <linux/fb.h>
#include <sys/ioctl.h>
#include <sys/mman.h>
        ...
        mNumLayers(0)
{
    ...
    // A1000: тройная буферизация.
    // Значение приходит из configstore, но на этом устройстве сервис не
    // поднимается («Cannot find entry ISurfaceFlingerConfigs»), поэтому всегда
    // берётся значение по умолчанию. При 2 получается двойная буферизация
    // (в dumpsys mMaxAcquiredBufferCount=1) и SurfaceFlinger ждёт освобождения
    // буфера, теряя кадры на слабом GPU. При 3 DisplayDevice включает тройную.
...
void SurfaceFlinger::doComposition() {
    ATRACE_CALL();
    bool repaintEverything = android_atomic_and(0, &mRepaintEverything);
    {
        /* A1000: the vendor OverlayComposer places partial damage at the wrong destination
         * origin (scanout dumps: frame translated right by 392 px), and is only ever correct
         * when the whole screen is redrawn. Set debug.sf.repaint_all=0 to restore stock
         * partial-damage behaviour. */
        char _ra[PROPERTY_VALUE_MAX];
        property_get("debug.sf.repaint_all", _ra, "1");
        if (_ra[0] != '0')
...
/* A1000: pan the framebuffer after a pure-GLES frame.
 * The vendor gralloc's fb_post() memcpy's the framebuffer target into the
 * reserved fb and returns without any ioctl, so the kernel never learns that a
 * new frame is there and DISPC keeps scanning the composer's buffer. Issuing
 * FBIOPAN_DISPLAY makes sprdfb_pan_display() -> ctrl->refresh() point DISPC at
 * fb->fix.smem_start, which is exactly where gralloc copied the frame.
 * Set debug.sf.a1000_pan=0 to disable. */
static void a1000_panFramebuffer()
{
    static int fbFd = -2;
    static int enabled = -1;
    static int panVsync = -1;
    static int dblbuf = -1;
    static int flipBuf = -1;
    static unsigned char *fbBase = NULL;
    static size_t frameBytes = 0;
    static unsigned int buffers = 1;
    static unsigned int nextBuf = 0;

    if (enabled < 0) {
        char v[PROPERTY_VALUE_MAX];
        property_get("debug.sf.a1000_pan", v, "1");
        enabled = (v[0] != '0');
    }
    if (!enabled) return;
    ...
    if (fbFd == -2) {
        ...
        ALOGE("A1000: pan fd=%d", fbFd);
    }
    if (fbFd < 0) return;

    struct fb_var_screeninfo vi;
    if (ioctl(fbFd, FBIOGET_VSCREENINFO, &vi) != 0) return;

    /* A1000: показываем кадр из свободного буфера, а не из того, куда пишут.
     * gralloc копирует кадр всегда в нулевой буфер и не умеет иначе, а мы до
     * сих пор его же и показывали (yoffset не трогали) — DISPC сканировал ту
     * самую память, по которой шёл memcpy, отсюда разрывы. Память под запас
     * есть и простаивала: virtual 480x2400 при видимых 480x800, то есть три
     * кадра. Переносим готовый кадр в следующий свободный буфер и показываем
     * его; нулевой остаётся под запись. Стоит лишний memcpy кадра.
     * debug.sf.a1000_dblbuf=0 возвращает прежнее поведение. */
    if (dblbuf < 0) {
        char v[PROPERTY_VALUE_MAX];
        property_get("debug.sf.a1000_dblbuf", v, "1");
        dblbuf = (v[0] != '0');
    }

    if (dblbuf && fbBase == NULL && vi.yres > 0) {
        struct fb_fix_screeninfo fi;
        if (ioctl(fbFd, FBIOGET_FSCREENINFO, &fi) == 0) {
            frameBytes = (size_t) fi.line_length * vi.yres;
            buffers = vi.yres_virtual / vi.yres;
            if (buffers > 1 && frameBytes > 0) {
                void *m = mmap(NULL, fi.smem_len, PROT_READ | PROT_WRITE,
                        MAP_SHARED, fbFd, 0);
                if (m == MAP_FAILED) {
                    ALOGE("A1000: mmap fb для двойной буферизации не удался");
                    dblbuf = 0;
                } else {
                    fbBase = (unsigned char *) m;
                    ALOGD("A1000: двойная буферизация включена, буферов %u по %zu байт",
                            buffers, frameBytes);
                }
            } else {
                dblbuf = 0;
    ...
    /* A1000: чередуем буферы без копирования.
     * Если вендорный gralloc в fb_post() учитывает текущий yoffset (а
     * стандартная реализация именно так и делает — пишет по base + offset),
     * то достаточно перед показом переключить окно вывода: gralloc положит
     * следующий кадр в свободный буфер, а DISPC будет сканировать предыдущий.
     * Это даёт честную двойную буферизацию даром, без memcpy кадра, который
     * оказался слишком дорогим для этого процессора.
     * Если же gralloc пишет всегда по smem_start, картинка начнёт мигать
     * старыми кадрами — тогда debug.sf.a1000_flip=0 возвращает как было. */
    if (flipBuf < 0) {
        char v[PROPERTY_VALUE_MAX];
        property_get("debug.sf.a1000_flip", v, "0");
        flipBuf = (v[0] != '0');
    }
    if (flipBuf && fbBase == NULL && vi.yres > 0) {
        struct fb_fix_screeninfo fi;
        if (ioctl(fbFd, FBIOGET_FSCREENINFO, &fi) == 0) {
            buffers = vi.yres_virtual / vi.yres;
            frameBytes = (size_t) fi.line_length * vi.yres;
            fbBase = (unsigned char *) 1;   /* просто отметка «размеры сняты» */
        }
    }
    if (flipBuf && buffers > 1) {
        nextBuf = (nextBuf + 1) % buffers;
        vi.yoffset = nextBuf * vi.yres;
        vi.activate = FB_ACTIVATE_VBL;      /* смена окна — по кадровому импульсу */
        if (ioctl(fbFd, FBIOPAN_DISPLAY, &vi) < 0) {
            static int moanedFlip = 0;
            if (moanedFlip < 3) { ALOGE("A1000: FBIOPAN_DISPLAY (flip) failed"); moanedFlip++; }
        }
        return;
    }

    if (dblbuf && fbBase != NULL && buffers > 1) {
        nextBuf = 1 + (nextBuf % (buffers - 1));
        memcpy(fbBase + (size_t) nextBuf * frameBytes, fbBase, frameBytes);
        vi.yoffset = nextBuf * vi.yres;
    } else {
        vi.yoffset = 0;
    }

    /* A1000: не ждём vsync второй раз.
     * Перед копированием кадра его уже ждёт libui_shim (перехват
     * compositionComplete + FBIO_WAITFORVSYNC), чтобы memcpy вендорного
     * gralloc уходил вперёд луча. Если здесь снова попросить FB_ACTIVATE_VBL,
     * кадр займёт ДВА периода vsync: замер показывал 37.2 мс на кадр при
     * периоде 16.6 мс, то есть 27 FPS вместо возможных ~50. К этому моменту
     * кадр уже лежит в буфере, и pan лишь сообщает ядру, что он готов.
     * debug.sf.a1000_pan_vsync=1 возвращает прежнее поведение. */
    if (panVsync < 0) {
        char v[PROPERTY_VALUE_MAX];
        property_get("debug.sf.a1000_pan_vsync", v, "0");
        panVsync = (v[0] != '0');
    }
    vi.activate = panVsync ? FB_ACTIVATE_VBL : FB_ACTIVATE_NOW;
    if (ioctl(fbFd, FBIOPAN_DISPLAY, &vi) < 0) {
        static int moaned = 0;
        if (moaned < 3) { ALOGE("A1000: FBIOPAN_DISPLAY failed"); moaned++; }
    }
}
...
void SurfaceFlinger::postFramebuffer()
{
    ...
    if (hwc.initCheck() == NO_ERROR) {
        ...
        /* A1000: a frame composed entirely by GLES goes to the vendor HWC's
         * framebuffer-target path, which copies it into the reserved fb but
         * never tells the kernel to scan it out. Do the pan ourselves. */
        if (hwc.hasGlesComposition(HWC_DISPLAY_PRIMARY) &&
                !hwc.hasHwcComposition(HWC_DISPLAY_PRIMARY)) {
            a1000_panFramebuffer();
        }
    }
        ...
        bool yswap, bool useIdentityTransform, Transform::orientation_flags rotation)
{
    ...
    extern void a1000_setSwizzleEnabled(bool enabled);   /* RenderEngine/ProgramCache.cpp */
    class SwizzleOff {
    public:
