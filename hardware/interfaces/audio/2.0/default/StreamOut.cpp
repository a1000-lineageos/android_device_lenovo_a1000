...
Result StreamOut::getPresentationPositionImpl(audio_stream_out_t* stream,
    ...
    // сообщать позицию воспроизведения. Это не ошибка, а такой же штатный
    // «позиции нет», как уже перечисленные EINVAL/ENODATA, но без него ALOGW
    // ниже срабатывал на КАЖДЫЙ буфер и заливал лог десятками строк в секунду.
    // На поведение это не влияет: analyzeStatus всё равно вернёт не-OK, и
    // фреймворк откатится на счёт записанных кадров.
