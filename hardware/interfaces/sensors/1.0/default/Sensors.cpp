...
Sensors::Sensors()
    ...
    // A1000: accept legacy HAL 1.0. The vendor blob reports
    // SENSORS_DEVICE_API_VERSION_1_0 (0x01000001) and CHECK_GE against 1_3 is
    // fatal, which killed the service at startup and hung the boot. Only the
    // core entry points are truly required; everything added after 1_0 is
    // guarded by a nullptr check at each call site, which is safe because
    // sensors_poll_device_1_t has a fixed size across the whole 1.x branch.
...
Return<Result> Sensors::batch(
    ...
    if (mSensorDevice->batch == nullptr) {
        return Result::INVALID_OPERATION;
    }
    return ResultFromStatus(
            mSensorDevice->batch(
...
Return<Result> Sensors::flush(int32_t sensor_handle) {
    if (mSensorDevice->flush == nullptr) {
        // A1000: flush() only exists from HAL 1_1 onwards.
        return Result::INVALID_OPERATION;
    }
