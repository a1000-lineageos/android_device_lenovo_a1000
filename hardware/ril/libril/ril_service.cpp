...
Return<void> RadioImpl::getDeviceIdentity(int32_t serial) {
    ...
    // пользовался фреймворк Android 5. Ответ разбирает тот же
    // radio::getDeviceIdentityResponse (ril_commands.h).
...
int radio::getIccCardStatusResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
         ...
         *
         * Раньше здесь стояли проверки вида
         *     p_cur->gsm_umts_subscription_app_index >= p_cur->num_applications
         * и любая из них отправляла статус карты в мусор с ошибкой
         * INVALID_RESPONSE. Их добавляли, чтобы не читать за границей массива
         * applications[], и это правильная забота — но лечение оказалось хуже
         * болезни: со вставленной картой блоб SPRD (сборка под Android 5)
         * присылает в одном из индексов своё значение, и Android объявлял оба
         * слота пустыми, хотя модем при этом уже видел сеть (25002).
         *
         * По стандарту RIL индекс -1 означает «такого приложения нет».
         * Поэтому: длину и указатель проверяем по-прежнему строго, а индексы
         * просто ПРИВОДИМ К -1, если они вне диапазона. Чтения за границей это
         * не допускает ровно так же, а карта остаётся видимой. */
        if (response == NULL || responseLen != sizeof(RIL_CardStatus_v6)) {
            RLOGE("getIccCardStatusResponse: Invalid response: responseLen %zu, ожидался sizeof(RIL_CardStatus_v6) %zu", responseLen, sizeof(RIL_CardStatus_v6));
            if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
        } else {
            int a1000_napp = p_cur->num_applications;
            if (a1000_napp < 0 || a1000_napp > RIL_CARD_MAX_APPS) {
                RLOGE("A1000: num_applications %d вне [0..%d], считаем 0",
                        a1000_napp, RIL_CARD_MAX_APPS);
                a1000_napp = 0;
            }
            RLOGE("A1000 SIM: card_state %d, pin_state %d, num_apps %d, "
                  "индексы gsm %d cdma %d ims %d",
                  p_cur->card_state, p_cur->universal_pin_state, a1000_napp,
...
int radio::getDeviceIdentityResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        // A1000_IMEI: мы подменили запрос 98 на 38 (GET_IMEI), и вендорная
        // libreference-ril_sp.so отвечает ОДНОЙ строкой цифр, а не массивом из
        // четырёх char*. По длине их не различить: 15 цифр + '\0' = 16 байт =
        // ровно 4 указателя, из-за чего rild уходил в ветку char** и падал в
        // strlen по адресу 0x37323533 (ASCII «3527» — начало самого IMEI).
        // Поэтому смотрим на содержимое: сплошные цифры длиной 14..16,
        // закрытые нулём, — это IMEI, а не таблица указателей.
        ...
        if (imeiPlainString) {
            // A1000_IMEISV: GET_IMEI отдаёт только сам IMEI, версии прошивки
            // модема (SVN) в блобе нет вообще — ни строки IMEISV, ни запроса 39,
            // а из AT она знает лишь +CGSN. Штатный reference-ril в такой же
            // ситуации возвращает константу "01"; здесь то же самое, но через
            // свойство ro.ril.imeisv, чтобы менять без пересборки.
            // ESN/MEID остаются пустыми: это поля CDMA, для GSM не нужны.
...
// 3 = SIM_LOCKED_OR_ABSENT, 4 = SIM_READY, 5..9 = RUIM/NV. Ни ril.h времён 8.1,
// ни HIDL android.hardware.radio@1.0 таких значений уже не знают — там только
// OFF(0), UNAVAILABLE(1) и ON(10). Если передать число как есть, фреймворк
// падает с RuntimeException "Unrecognized RadioState" и навсегда остаётся с
// RADIO_OFF (в дампе телефонии — POWER_OFF при живом AT+CFUN=1).
// Все устаревшие значения означают ВКЛЮЧЁННОЕ радио и различаются лишь
// готовностью SIM, о которой фреймворк спрашивает отдельно (GET_SIM_STATUS),
// поэтому схлопываем их в ON.
static RadioState convertVendorRadioState(int vendorState) {
    switch (vendorState) {
        ...
        default:
            RLOGD("convertVendorRadioState: устаревшее состояние %d -> ON", vendorState);
...
void convertRilSignalStrengthToHal(void *response, size_t responseLen,
    ...
    if (responseLen == sizeof(RIL_SignalStrength_v8)) {
        ...
        /* A1000: блоб присылает КОРОТКУЮ структуру.
         *
         * libreference-ril_sp.so собран под Android 5 и отдаёт
         * RIL_SignalStrength_v5 (28 байт) или _v6 (48), а здесь ждут _v8 (52)
         * или _v10 (56). Из-за этого вся индикация уровня сигнала
         * отбрасывалась с "invalid response", и в телефонии оставались
         * 99/-1 — «неизвестно», сколько бы модем ни отдавал.
         *
         * Короткие версии — это байтовые ПРЕФИКСЫ v10: GW, CDMA и EVDO лежат
         * по тем же смещениям, у v6 совпадают и первые пять полей LTE.
         * Поэтому копируем что пришло в обнулённую v10, а недостающие поля
         * заполняем «неизвестно» и отдаём обычному разбору. */
