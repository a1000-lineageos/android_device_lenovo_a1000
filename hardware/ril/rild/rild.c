...
int main(int argc, char **argv) {
    ...
    // A1000: номер слота = clientId + 1.
    // Spreadtrum запускает ПО ПРОЦЕССУ rild НА СЛОТ (wril-daemon -c 0 и
    // wril-daemon1 -c 1), а имена HIDL-сервисов — slot1 и slot2. Исходный код
    // клеил "slot" + clientId, то есть при -c 1 получалось снова "slot1":
    // оба демона регистрировались под одним именем и затирали друг друга, а
    // RILJ [SUB1] не находил slot2 (NoSuchElementException).
    // Сам -c при этом менять нельзя — его же значение уходит в вендорную
    // RIL_Init, и по нему она выбирает AT-каналы: 0 -> /dev/CHNPTYW0..2,
    // 1 -> /dev/CHNPTYW3..5.
    snprintf(ril_service_name, MAX_SERVICE_NAME_LENGTH, "%s%d",
            ril_service_name_base, atoi(clientId) + 1);
