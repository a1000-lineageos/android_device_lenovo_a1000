...
bool NetlinkUtils::GetStationInfo(uint32_t interface_index,
  ...
  // A1000: драйвер может не отдавать часть полей станции. Раньше любое
  // отсутствующее поле роняло весь GetStationInfo в false, и вместе со
  // счётчиками терялся RSSI — WifiStateMachine оставался с -127, а SystemUI
  // рисовал серую иконку без делений при живом подключении. Обязателен
  // только SIGNAL; остальное — необязательная телеметрия.
  int32_t tx_good = 0, tx_bad = 0;
  if (!sta_info.GetAttributeValue(NL80211_STA_INFO_TX_PACKETS, &tx_good)) {
    LOG(WARNING) << "No NL80211_STA_INFO_TX_PACKETS from driver, assuming 0";
    tx_good = 0;
  }
  if (!sta_info.GetAttributeValue(NL80211_STA_INFO_TX_FAILED, &tx_bad)) {
    LOG(WARNING) << "No NL80211_STA_INFO_TX_FAILED from driver, assuming 0";
    tx_bad = 0;
  }
  int8_t current_rssi;
  ...
  if (!sta_info.GetAttribute(NL80211_STA_INFO_TX_BITRATE, &tx_bitrate_attr) ||
      !tx_bitrate_attr.GetAttributeValue(NL80211_RATE_INFO_BITRATE32,
                                        &tx_bitrate)) {
    LOG(WARNING) << "No NL80211_STA_INFO_TX_BITRATE from driver, assuming 0";
    tx_bitrate = 0;
  }
